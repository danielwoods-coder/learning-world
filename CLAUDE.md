...
A personal collection of standalone learning exercises across HTML/CSS, JavaScript, Python, SQL, and Bash. There is **no unified build system, no test suite, and no package manager** — each subdirectory is independent and is run directly with the language's native tooling.
...
- [docs/](docs/) — HTML/CSS standalone pages (formerly in `HTML/`). Paired `name.html` + `name.css` share base names. Run by opening the file in a browser (or via a Live Server extension).
...
# HTML/CSS pages — open in browser, e.g.
xdg-open docs/cafe-menu.html
...
No ESLint config is checked in — no `eslint.config.*`, `.eslintrc*`, or `package.json` exists anywhere in the tree. Don't run `npx eslint .`; it will error on missing config. If linting is genuinely needed, add `eslint.config.js` under [JavaScript/](JavaScript/) first.
